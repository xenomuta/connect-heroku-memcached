  ...
  module.exports = function(connect) {
    ...
    return HerokuMemcachedStore = (function(_super) {
      ...
      function HerokuMemcachedStore(options) {
        var port, server, servers, _dummy, _i, _len, _ref1, _ref2;
        ...
        for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
          server = _ref1[_i];
          if (/^([^:]+):(\d+)$/.test(server)) {
            _ref2 = server.match(/^([^:]+):(\d+)$/), _dummy = _ref2[0], server = _ref2[1], port = _ref2[2];
          } else {
            port = 11211;
          }
      ...
      HerokuMemcachedStore.prototype.get = function(sid, callback) {
        return client.get(sid, function(err, val, flags) {
          ...
          try {
            session = JSON.parse(val != null ? val.toString() : void 0);
          } catch (_error) {}
          if (session == null) {
            session = ((val != null ? val.length : void 0) > 0 ? val.toString() : {
              cookie: {}
            });
          }
      ...
      HerokuMemcachedStore.prototype.set = function(sid, session, callback) {
        return client.set(sid, JSON.stringify(session), callback);
      };

      HerokuMemcachedStore.prototype.destroy = function(sid, callback) {
        return client["delete"](sid, callback);
      };
